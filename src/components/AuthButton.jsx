...
import { signUp, signInWithPassword, signInWithGoogle, signOut, resetPassword } from '../auth';
...
export default function AuthButton({ user, userProfile, userTier, onRefreshUser, forceShowSignup, onModalClose }) {
  ...
  async function handleSignUp(e) {
    ...
    try {
      ...
      setMessage('✅ Account created! Check your email for verification link. Welcome to SayEasy!');
      ...
      {showAuthModal && (
        <div className="fixed inset-0 bg-purple-100 bg-opacity-80 flex items-center justify-center z-50 p-4">
          <div className="bg-gray-800 rounded-2xl p-6 w-full max-w-md border border-gray-700">
            ...
            <form onSubmit={authMode === 'signin' ? handleSignIn : authMode === 'signup' ? handleSignUp : handleForgotPassword} className="space-y-4">
              {authMode === 'signup' && (
                <input
                  type="text"
              ...
              >
                {loading ? 'Processing...' : (authMode === 'signin' ? 'Sign In' : authMode === 'signup' ? 'Create Account' : 'Send Reset Email')}
              </button>
            </form>
            ...
            <div className="mt-4 text-center space-y-2">
              {authMode === 'signin' && (
                <button
                  ...
                  className="text-blue-400 hover:text-blue-300 text-sm transition-colors block mx-auto"
                >
                  Forgot Password?
                </button>
              )}
              <button
                onClick={() => setAuthMode(authMode === 'signin' ? 'signup' : authMode === 'signup' ? 'signin' : 'signin')}
                className="text-blue-400 hover:text-blue-300 text-sm transition-colors"
              >
                {authMode === 'signin' ? "Don't have an account? Create one" : 
            ...
            {message && (
              <div className={`mt-4 p-3 rounded-lg text-center ${
                message.includes('Welcome') || message.includes('created') || message.includes('✅') 
